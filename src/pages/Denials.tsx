...
const Denials = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'payout_cases', filter: 'status=eq.denied' }, () => {
        fetchDenials();
        toast({ title: "New Denial Added", description: "A new payout denial has been submitted", variant: "destructive" });
      })
      .subscribe();
  ...
  const fetchDenials = async () => {
    try {
      ...
      toast({ title: "Error fetching denials", description: error.message, variant: "destructive" });
    }
  };
