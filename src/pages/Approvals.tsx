...
import { CheckCircle, Calendar, DollarSign, LayoutGrid, List, Twitter } from "lucide-react";
            ...
            <motion.div key="table" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
              <div className="glass rounded-lg overflow-x-auto">
                <Table>
                  ...
                  <TableBody>
                    {visibleCases.map((c) => (
                      <TableRow key={c.id} className={`border-border hover:bg-secondary/30 transition-colors border-l-2 ${c.verification_status === 'disputed' ? 'border-l-destructive opacity-80' : 'border-l-success/50'}`}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <CaseVoteButtons caseId={c.id} ownerId={c.user_id} upvotes={c.upvotes_count || 0} flags={c.flags_count || 0} onVoted={fetchApprovals} compact />
                          </div>
                        </TableCell>
            ...
            <motion.div key="card" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {visibleCases.map((payoutCase) => (
                  <Card key={payoutCase.id} className={`glass p-6 transition-smooth hover:scale-105 hover:glow-approval ${payoutCase.verification_status === 'disputed' ? 'border-destructive/50' : ''}`}>
                    ...
                    <div className="mt-4 pt-4 border-t border-border flex items-center justify-between gap-2">
                      ...
                      <CaseVoteButtons caseId={payoutCase.id} ownerId={payoutCase.user_id} upvotes={payoutCase.upvotes_count || 0} flags={payoutCase.flags_count || 0} onVoted={fetchApprovals} compact />
                    </div>
                  </Card>
                ))}
              </div>
